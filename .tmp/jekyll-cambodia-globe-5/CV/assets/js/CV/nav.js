(function () {
    ...
    var pendingInitialHashRafId = null;
    ...
    function stopPendingInitialHashAlignment() {
        if (pendingInitialHashRafId === null) {
            return;
        }

        window.cancelAnimationFrame(pendingInitialHashRafId);
        pendingInitialHashRafId = null;
    }

    function alignToCurrentHash(options) {
        ...
        scrollToTarget(hash, options);
    }

    function queueInitialHashAlignment() {
        var rafCount = 0;

        stopPendingInitialHashAlignment();

        function step() {
            rafCount += 1;

            if (rafCount < 3) {
                pendingInitialHashRafId = window.requestAnimationFrame(step);
                return;
            }

            pendingInitialHashRafId = null;
            alignToCurrentHash({ immediate: true });
        }
    ...
    window.addEventListener("load", queueInitialHashAlignment, { once: true });
    window.addEventListener("pageshow", function (event) {
        if (event.persisted) {
            queueInitialHashAlignment();
        }
    });
    window.addEventListener("hashchange", function () {
        alignToCurrentHash({ immediate: true });
    });
    document.addEventListener("cv-loader-exit-start", queueInitialHashAlignment);
