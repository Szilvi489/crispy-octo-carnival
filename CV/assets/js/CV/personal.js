(function () {
    ...
    function buildPersonalItemConfigs() {
        personalItemConfigs = personalItems.map(function (item) {
            ...
            return {
                element: item,
                baseOpacity: clamp(parseNumber(item.dataset.baseOpacity, 1), 0, 1),
                start: start,
                end: end,
    ...
    function renderPersonalItems(sectionProgress) {
        ...
        personalItemConfigs.forEach(function (config) {
            ...
            var x = lerp(config.fromX, config.toX, easedProgress) * (viewportWidth / 100);
            var y = lerp(config.fromY, config.toY, easedProgress) * (viewportHeight / 100);
            ...
            var opacity = clamp(
                lerp(config.fromOpacity, config.toOpacity, easedProgress) * config.baseOpacity * sectionAlpha,
                0,
                1
            );
